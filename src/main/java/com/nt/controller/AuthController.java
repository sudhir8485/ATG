...
users.addAll(repo.findByEmailAndPassword(username,password));

if(users != null && !users.isEmpty()){

    // pick the first user matching the selected role (handles accidental duplicates)
    ...
    if(role.equalsIgnoreCase(user.getRole())){
        // store basic user info in session for downstream pages
        ...
        // set Spring Security authentication so URL protections apply
        ...
        if("ADMIN".equalsIgnoreCase(role)){
            ...
            session.setAttribute("currentFacultyKey", user.getName() != null ? user.getName() : user.getUsername());
