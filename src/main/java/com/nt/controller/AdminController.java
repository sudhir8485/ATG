...
public String adminDashboard(Model model){
    ...
    java.util.Map<String, Long> facultyByDept = faculty.stream()
            .filter(f -> f.getDepartment() != null)
            .collect(java.util.stream.Collectors.groupingBy(User::getDepartment, java.util.stream.Collectors.counting()));
...
public String assignSubjectFaculty(@PathVariable int id,
...
public String saveClassroom(@RequestParam String room,
...
public String updateClassroom(@PathVariable int id,
...
public String saveHod(
    ...
    deptRepo.findAll().stream()
            .filter(d -> department.equalsIgnoreCase(d.getName()))
            .findFirst()
            .ifPresent(d -> {
                d.setHod(name);
                deptRepo.save(d);
...
public String updateHod(@PathVariable int id,
                        @RequestParam String name,
                        @RequestParam String department,
                        @RequestParam String email,
...
@RequestParam(required=false) List<String> password,
...
@RequestParam(required=false) List<String> maxLecturesPerWeek,
RedirectAttributes redirect){
...
public String updateFaculty(@PathVariable int id,
                            @RequestParam String name,
...
@RequestParam(required=false) Integer maxLecturesPerWeek,
RedirectAttributes redirect){
...
@RequestParam String room,
@RequestParam String lectureType,
...
List<Timetable> list = timetableRepo.findByDeletedFalse();
    ...
    if(department != null && !department.isBlank()){
        list = list.stream()
                .filter(t -> t.getDepartment() != null && t.getDepartment().equalsIgnoreCase(department))
                .toList();
    }
    ...
    List<String> departments = timetableRepo.findByDeletedFalse().stream()
            .map(Timetable::getDepartment)
            .filter(d -> d != null && !d.isBlank())
            .distinct()
            .sorted()
            .toList();
    List<String> classes = timetableRepo.findByDeletedFalse().stream()
...
public ResponseEntity<byte[]> exportLoadDistribution(
        @RequestParam(required=false) String department,
        @RequestParam(required=false) String className){
    ...
    try{
        ...
        Paragraph college = new Paragraph("ASM's Pawar College of Engineering & Research, Parvat, Pune", collegeFont);
        ...
        Paragraph note = new Paragraph("Note: Counts represent scheduled hours per week by faculty and year bucket. TH = Theory, PR = Practical/Lab.", smallFont);
...
public ResponseEntity<byte[]> exportPdf(
    ...
    List<String> slotLabels = slots.stream().map(SlotDef::label).toList();

    // group per class/division so each sheet matches the admin timetable view
    Map<String, List<Timetable>> grouped = new LinkedHashMap<>();
    for(Timetable t : entries){
        String label = buildClassLabel(t);
        grouped.computeIfAbsent(label, k -> new java.util.ArrayList<>()).add(t);
    }

    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    Document doc = new Document(PageSize.A4.rotate(), 20, 20, 26, 26);
    try{
        PdfWriter.getInstance(doc, baos);
        doc.open();
        ...
        for(Map.Entry<String, List<Timetable>> group : grouped.entrySet()){
            ...
            String classLabel = group.getKey();
            String dept = list.stream()
                    .map(Timetable::getDepartment)
                    .filter(java.util.Objects::nonNull)
                    .filter(s -> !s.isBlank())
                    .findFirst()
                    .orElse((department != null && !department.isBlank()) ? department : "Department");

            Paragraph deptLine = new Paragraph("Department of " + dept + " - Timetable (" + academicYear + ")", titleFont);
            deptLine.setAlignment(Element.ALIGN_CENTER);
            deptLine.setSpacingAfter(4f);
            doc.add(deptLine);

            Paragraph classLine = new Paragraph(classLabel, headerFont);
            classLine.setAlignment(Element.ALIGN_CENTER);
            classLine.setSpacingAfter(12f);
            doc.add(classLine);

            int cols = slotLabels.size() + 1; // Day + slots (Class column removed)
            PdfPTable table = new PdfPTable(cols);
            table.setWidthPercentage(100);
            float[] widths = new float[cols];
            widths[0] = 11f;
            for(int i=1;i<cols;i++){
                widths[i] = (float)(89.0 / slotLabels.size());
            }
            ...
            for(int i=0; i<slotLabels.size(); i++){
                String lbl = slotLabels.get(i);
                PdfPCell h = new PdfPCell(new Phrase(lbl, headerFont));
                h.setHorizontalAlignment(Element.ALIGN_CENTER);
                h.setVerticalAlignment(Element.ALIGN_MIDDLE);
                h.setPadding(6f);
                if(slots.get(i).isBreak()){
                    h.setBackgroundColor(new java.awt.Color(239, 242, 247));
                }else{
         ...
         // Add merged break cells (rowspan = all days) for first day row upfront
            // These span all day rows so we skip break columns inside the day loop
            java.util.Set<Integer> breakColIndices = new java.util.LinkedHashSet<>();
            for(int i=0;i<slots.size();i++){
                if(slots.get(i).isBreak()) breakColIndices.add(i);
            }
            ...
            for(String day : days){
                ...
                for(int i=0;i<slots.size();i++){
                    ...
                    if(ts.isBreak()){
                        if(day.equals(days.get(0))){
                            // Only add once for first day, spanning all day rows
                            ...
                            breakCell.setPadding(8f);
                            breakCell.setRowspan(days.size());
                            breakCell.setBackgroundColor(new java.awt.Color(239, 242, 247));
                            table.addCell(breakCell);
                        }
                        // Skip for all other days (rowspan covers them)
                        continue;
                    }
                    ...
                    for(int idx=0; idx<matches.size(); idx++){
                        ...
                        if(t.getFaculty() != null && !t.getFaculty().isBlank()){
                            cellPara.add(com.lowagie.text.Chunk.NEWLINE);
                            cellPara.add(new com.lowagie.text.Chunk("(" + toInitials(t.getFaculty()) + ")", bodyFont));
                        }
                        if(isPracticalSlot(t.getLectureType(), t.getSubject())){ anyPractical = true; }
                    }

                    PdfPCell detail = new PdfPCell(cellPara);
                    detail.setPadding(8f);
                    if(anyPractical){
            ...
            Paragraph note = new Paragraph("* Exported from Admin > View Timetable. Break slots are shaded. Multiple entries in a cell are stacked line by line.", noteFont);
            note.setSpacingBefore(8f);
            doc.add(note);
        }
...
private String getBreakLabel(SlotDef ts){
    ...
    if(dur >= 45) return "LUNCH BREAK";
    if(dur >= 15) return "SHORT BREAK";
    return "BREAK";
}

private List<Timeslot> insertBreakSlots(List<Timeslot> slots){
    if(slots == null || slots.size() < 2) return slots;
...
private String safeListVal(List<String> list, int idx){
    if(list == null || idx >= list.size() || idx < 0) return "";
...
private String matchSlot(List<String> slots, String slotVal){
    ...
    return slots.stream()
            .filter(s -> normalizeKey(s).equals(key)
                    || (minutes != null && minutes.equals(parseSlotStart(s))))
            .findFirst()
            .orElse(null);
}
...
private int slotStartMinutes(String slot){
    ...
    if(parts.length == 0) return Integer.MAX_VALUE;
...
private Integer parseSlotStart(String slot){
    ...
    try{
        ...
        return java.time.LocalTime.parse(start.replace(".", ""), fmt).getHour()*60 + java.time.LocalTime.parse(start.replace(".", ""), fmt).getMinute();
    }catch(Exception ignore){}
...
private List<SlotDef> getExportSlots(){
    ...
    if(stored != null && !stored.isEmpty()){
        return stored.stream()
                .sorted(java.util.Comparator.comparingInt(ts -> parseSlotStart(ts.getStartTime()) == null ? Integer.MAX_VALUE : parseSlotStart(ts.getStartTime())))
                .map(ts -> new SlotDef(
                        (ts.getStartTime() != null ? ts.getStartTime() : "") + " - " + (ts.getEndTime() != null ? ts.getEndTime() : ""),
                        ts.getStartTime(),
                        ts.getEndTime(),
                        ts.isBreak()
                ))
    ...
    return java.util.List.of(
            new SlotDef("09:00 AM - 10:00 AM", "09:00", "10:00", false),
...
private List<Timetable> filterTimetables(String department, String className){
    ...
    if(department != null && !department.isBlank()){
        ...
        list = list.stream()
                .filter(t -> t.getDepartment() != null && matchesDept(t.getDepartment(), deptKey))
                .toList();
    }
    if(className != null && !className.isBlank()){
...
private static class FacultyLoad{
    ...
    String getEntries(int year, String type){
        ...
        if(subjMap == null || subjMap.isEmpty()) return "";
